...
def search():
    ...
    if query:
        mobiles_query = mobiles_query.filter(Mobile.name.ilike(f'%{query}%'))
    if condition:
    ...
    if brand:
        mobiles_query = mobiles_query.filter(Mobile.brand.ilike(f'%{brand}%'))

    mobiles = mobiles_query.order_by(Mobile.created_at.desc()).all()
    return render_template('search.html', mobiles=mobiles, conditions=CONDITION_CHOICES)
...
def add_mobile():
    if request.method == 'POST':
        ...
        return redirect(url_for('admin_dashboard'))
    return render_template('admin/add_mobile.html', conditions=CONDITION_CHOICES, brands=INDIAN_BRANDS)
...
@app.route('/admin/<string:type>/delete/<int:id>')
@login_required
def delete_item(type, id):
    if type == 'mobile':
        item = Mobile.query.get_or_404(id)
    elif type == 'broadcast':
        item = Broadcast.query.get_or_404(id)
    elif type == 'community':
        item = CommunityPost.query.get_or_404(id)
    else:
